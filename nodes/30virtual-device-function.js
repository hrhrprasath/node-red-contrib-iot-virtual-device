 ...
 **/
 ...
 //	"use strict";
	...
	/*
	 * device function node (modified function node)
	 */
	...
	function deviceFunction(n) {
        ...
        var device = (node.schema) ? deviceManager.getDevice(node.schema.deviceType, deviceId) : deviceManager.getDevicebyId(deviceId);
				...
				var deviceContext = device.properties;

				try {
					var start = process.hrtime();
					context.msg = msg;
					_.each(deviceContext, function(value, key){
						if(context[key])
							node.error(key + " is already in function contexted and cannot be used as a device property", msg);
						else
							context[key] = value;
					...
					context.msg = msg;
					_.each(deviceContext, function(value, key){
						if(deviceContext[key] !== context[key])
							deviceContext[key] = context[key];
						delete context[key];
					});
	...
	RED.nodes.registerType("device function", deviceFunction);
